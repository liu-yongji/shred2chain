...
int nchangechain2(std::map<int,ChainBag> &map_Chain,string &desf,string schr,int n19,int n38,int &nChain)
{
  ...
  int np1 = 0;
	...
	int nChainNum = 1;
	string sChainInfo = "";
	//string sSsum = int2sting(n19);
	//string sDsum = int2sting(n38);
 ...
 {  
	  ...
	  if(sLastand == "") 
        ...
        ndeslstE =  nED;	
		    ...
		    {
		    		 sChainInfo = "";
             sChainInfo = "chain 1 " + schr + " " + int2sting(n19) + " + " + sSrcS + " " + int2sting(nsrcLstE) + " " + schr ;
            if(ndesS > ndesE )
            {
             int nT = ndesS;
             ndesS = ndesE;
             ndesE = nT;
		  	      ...
		  	      //sChainInfo += " " + int2sting(n38) + " " + sLastand + " " + int2sting(ndesS) + " " + int2sting(ndeslstE) + " " + int2sting(nChain++) +  "\n";
		  	      sChainInfo += " " + int2sting(n38) + " " + sLastand + " " + int2sting(ndesS) + " " + int2sting(ndesE) + " " + int2sting(nChain++) +  "\n";
		  	      sChainInfo += ssub + "\n\n";
		  	     // cout << sChainInfo ;
		  	      desf += sChainInfo;
		  	     sLastand =  "";
        ...
        nG2 = nES - 1 - ndeslstE;
		  	...
		  	if(nG2 < 0) // 换链
		  	 ...
		  	 ndeslstE =  nED;
		  	 nsrcLstE = nSEND;
		     ssub += " " + int2sting(nG1) + " " + int2sting(nG2);
		     ssub += "\n" + int2sting(nGap);	
		  }
		  else // 换链
		  {
	...
	else //说明此段不是对齐的
	{
		...
		for (int i = 0;i<nlen1;i++)
		{
			...
			if (s == '-'&& d != '-')// 37号上偏
			{
				...
				{
					stRE += int2sting(nSs) + " ";
					stRE += int2sting(nSrcEND) + " ";
					stRE += sSTRAND  + " ";
					stRE += int2sting(nDs) + " ";
					stRE += int2sting(nDesEND-1) + "\n";
					nSs = nSrcEND+1;
					nDs = nDesEND+1;
					nSs1 = 0;
					nDs1 = 0;
			...
			else if (s!= '-'&& d == '-')//38号偏移
			{
				...
				if (nDs ==nDesEND && nSrcEND != nSs && cLD != '-') //单对单
				{
					...
					stRE += int2sting(nSs) + " ";
					stRE += int2sting(nSrcEND-1) + " ";
					stRE += sSTRAND  + " ";
					stRE += int2sting(nDs) + " ";
					stRE += int2sting(nDesEND) + "\n";
					nSs = nSrcEND+1;
					nDs = nDesEND+1;
					nSs1 = 0;
					nDs1 = 0;
...
string   PickPairsC(string &sQUERY,string &sINDEX,string &sREFERENCE,string &sSTRAND,int nPosbase)//成功抽取反向互补插入
{
	...
	if (sINDEX.find("-") == -1)//说明此段是对齐的
	{
		stRE = int2sting(Y-nSs) + " ";
		stRE += int2sting(Y-nSe) + " ";
		stRE += sSTRAND  + " ";
		stRE += int2sting(nDs) + " ";
		stRE += int2sting(nDe)+ "\n";
	}
